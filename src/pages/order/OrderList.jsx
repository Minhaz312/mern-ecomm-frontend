...
export default function OrderList() {
  ...
  const getAllOrder = () => {
    axios.get(`${apiUrl}/order/get/all`).then(res=>{
  ...
  const acceptOrder = orderId => {
    axios.put(`${apiUrl}/order/accept`,{orderId:orderId}).then(res=>{
  ...
  const handleConfirmDelivery = id => {
    axios.put(`${apiUrl}/order/delivery/confirm`,{orderId:id}).then(res=>{
      if(res.status === 200 && res.data.success === true) {
        getAllOrder()
      }
    }).catch(err=>{
  ...
  const productListColumn = [
    ...
    {name:"Action",cell:row=><div>
      {
        row.accepted===false?<>
          <button className='btn btn-sm btn-info' onClick={handleShowOrderModal.bind(this,row)}>View</button>
          <button onClick={acceptOrder.bind(this,row._id)} className='btn btn-sm mx-1 rounded btn-success'>Accept</button>    
		      <button className='btn btn-sm rounded btn-danger'>Reject</button>
        </>:<>
          <button className='btn btn-sm me-1 btn-info' onClick={handleShowOrderModal.bind(this,row)}>View</button>
          <button className='btn btn-sm rounded btn-success' onClick={handleConfirmDelivery.bind(this,row._id)}>Delivery Confirm</button>
        </>
      }
