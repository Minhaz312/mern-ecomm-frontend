...
export default function Modal({children,loading=false,show=false,footer=true,title="Modal Title",onAction=()=>{},onHide=()=>{},className="bg-white",btnTitle="add"}) {
  return (
    <div className={`bg-black/30 fixed backdrop-blur-[5px] left-0 top-0 bottom-0 right-0 z-[10000] h-full w-full sm:w-full flex justify-center items-center ${show?"block":"hidden"}`}>
        <div className={`min-h-[200px] sm:min-w-[450px] min-w-[80%] max-w-[95%] animate-zoomIn p-3 flex flex-col justify-between ${className} text-slate-900 bg-slate-100 md:max-w-[60%] sm:max-w-[70%]`}>
